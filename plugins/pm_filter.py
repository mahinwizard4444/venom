...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏪ BACK", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"📃 Pages {round(int(offset)/7)+1} / {round(total/7)}", callback_data="pages")]
        )
    elif off_set is None:
        btn.append([InlineKeyboardButton("NEXT ⏩", callback_data=f"next_{req}_{key}_{n_offset}"), InlineKeyboardButton(f"📘 {round(int(offset)/7)+1} / {round(total/7)}", callback_data="pages")])
    else:
        btn.append(
            [
...
📫 Yᴏʀ Fɪʟᴇ ɪꜱ Rᴇᴀᴅʏ 👇
📂 Mᴏᴠɪᴇ Nᴀᴍᴇ : {title}
⚙️ Mᴏᴠɪᴇ Sɪᴢᴇ : {size}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📥 Download Link 📥", url=f"{filess.link}")
                        ],[
                        InlineKeyboardButton("⚠️ Can't Access❓ Click Here ⚠️", url=f"https://t.me/{SEND_USERNAME}")
                        ]]))
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ɪɴꜰᴏ', callback_data='infohelp')
            ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"📘 1/{round(int(total_results)/7)}",callback_data="pages"), InlineKeyboardButton(text="NEXT ⏩",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
