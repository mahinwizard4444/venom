...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        ...
        btn.append([InlineKeyboardButton("NEXT ⏩", callback_data=f"next_{req}_{key}_{n_offset}"), InlineKeyboardButton(f"📘 {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")])
    else:
        btn.append(
            [
